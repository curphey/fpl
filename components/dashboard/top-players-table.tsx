...
function getSortedPlayers(players: EnrichedPlayer[], tab: Tab): EnrichedPlayer[] {
...
export function TopPlayersTable({
  ...
  const sorted = getSortedPlayers(players, activeTab);

  const columns: Column<EnrichedPlayer>[] = [
    {
      key: 'rank',
      header: '#',
      className: 'w-8',
    ...
    },
    {
      ...
      render: (p) => (
        <div>
          ...
          <span className="ml-2 text-xs text-fpl-muted">{p.team_short_name}</span>
        </div>
      ),
    },
    {
