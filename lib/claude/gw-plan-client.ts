...
10. Formation and lineup: After considering transfers and substitutions, recommend the optimal formation (e.g. 4-3-3, 3-5-2) and provide the final starting XI and bench order. The formation must have at least 1 GK, 3 DEF, 2 MID, 1 FWD in the starting XI. Explain why the formation suits the upcoming fixtures.
...
Respond with JSON matching this schema exactly:
{
  ...
  "lineupPlan": {
    ...
    "benchOrder": [<4 player IDs — bench GK LAST>]
  },
  ...
  try {
    ...
    return {
      ...
      lineupPlan: parsed.lineupPlan
        ? {
            startingXI: (parsed.lineupPlan.startingXI ?? []).map(
              (id: number) => ({
                id,
                name: `Player ${id}`,
              }),
            ),
            benchOrder: (parsed.lineupPlan.benchOrder ?? []).map(
              (id: number) => ({
                id,
                name: `Player ${id}`,
              }),
